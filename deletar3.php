...
$id = $_GET['id'];
$stmt = $pdo->prepare('SELECT * FROM respostas WHERE id_resposta = ?');
$stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD']== 'POST') {
    $stmt = $pdo->prepare('DELETE FROM respostas WHERE id_resposta = ?');
    $stmt->execute([$id]);
...
<div align="center" class="quadrado-arredondado3">
    ...
    <div class="letrinha">Deletar Resposta</div>
    <p>Tem certeza que deseja deletar a Resposta? <?php echo $appointment["resposta_selecionada"]; ?></p>
    <form method="post">
    <button class="btao1"><a type="submit">Sim</a></button>
        <button class="btao1"><a href="visu.php">Não</a></button>
